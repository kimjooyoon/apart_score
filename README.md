...
## 🎯 프로젝트 비전: 주관적 선호를 객관적 평가로

**"이 건물이 더 좋은 것 같다"라는 느낌을 체계적으로 숫자로 변환하는 투명한 평가 시스템**
...
### 🔄 데이터 플로우
1. **입력**: 아파트 점수 맵 + 가중치 맵
2. **변환**: 맵 → 고정 크기 배열 (ScoreArray/WeightArray)
...
#### 📊 **실시간 성능 보장**
- **처리 속도**: < 0.1초 (단일 아파트 계산)
- **메모리 사용**: < 1MB (최적화된 데이터 구조)
- **확장성**: 동시 요청 처리 가능
